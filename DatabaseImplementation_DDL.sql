...
UPDATE CUSTOMER
SET customer_phone = ENCRYPTBYKEY(KEY_GUID('customer_phone_encryption_key'), customer_phone);
-- Select the encrypted values
SELECT CustomerID, customer_name, customer_email,
       ENCRYPTBYKEY(KEY_GUID('customer_phone_encryption_key'), customer_phone) as Customer_phone
FROM CUSTOMER;

-- Select the decrypted values
SELECT CustomerID, customer_name, customer_email,
       CONVERT(VARCHAR(50), DECRYPTBYKEY(customer_phone)) AS customer_phone
FROM CUSTOMER;
